...
post() { # $1 = Store header, $2 = json body
...
echo "== availableStores (Store: $STORE_EN) =="
post "$STORE_EN" '{"query":"{ availableStores(useCurrentGroup:false){ store_code store_name locale is_default_store base_currency_code default_display_currency_code base_url secure_base_url base_media_url } }"}'
echo; echo
...
post "$STORE_EN" "$INTROSPECTION_QUERY" > "$OUT_DIR/schema.introspection.json"
