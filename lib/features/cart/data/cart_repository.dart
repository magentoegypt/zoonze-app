...
class CartRepository {
  ...
  Future<String> createGuestCart() async {
    final data = await _run(CartQueries.createEmptyCart, const {}, mutation: true);
    final id = data['createEmptyCart'] as String?;
    if (id == null || id.isEmpty) {
      throw const Failure(FailureKind.server, detail: 'createEmptyCart was null');
    }
    return id;
  }

  Future<String?> customerCartId() async {
    final data = await _run(CartQueries.customerCart, const {}, mutation: false);
    return (data['customerCart'] as Map<String, dynamic>?)?['id'] as String?;
  }

  Future<Cart> getCart(String cartId) async {
    final data = await _run(CartQueries.getCart, {'cartId': cartId},
        mutation: false);
    return _parseCart(data['cart'] as Map<String, dynamic>?, fallbackId: cartId);
  }
  ...
  Future<Cart> addProducts(
    ...
    final data = await _run(
      CartQueries.addProducts,
      {'cartId': cartId, 'items': items},
      mutation: true,
    );
    ...
    return _parseCart(result?['cart'] as Map<String, dynamic>?, fallbackId: cartId);
  }

  Future<Cart> updateItem(String cartId, String uid, int quantity) async {
    final data = await _run(
      CartQueries.updateItems,
      {
        'cartId': cartId,
        'items': [
  ...
  Future<Cart> removeItem(String cartId, String uid) async {
    final data = await _run(
      CartQueries.removeItem,
      {'cartId': cartId, 'uid': uid},
      mutation: true,
    );
    return _parseCart(
  ...
  Future<Cart> mergeCarts(String source, String destination) async {
    ...
    return _parseCart(data['mergeCarts'] as Map<String, dynamic>?,
