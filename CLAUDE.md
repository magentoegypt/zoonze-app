...
2. **Payment integration (per gateway):** _(Resolved: **native SDK** for Phase 3.)_ The app integrates **N-Genius** (native `payment-sdk-android` / iOS `NISdk` via a `MethodChannel`) and **Tabby** (official `tabby_flutter_inapp_sdk`; presented via the redirect engine until the SDK is wired) behind a provider-agnostic `PaymentGateway` seam. **Backend dependency stands:** neither Magento extension exposes the SDK session reference to a headless client — add the recommended `paymentSession(order_number)` GraphQL resolver (one resolver, both gateways). The Dart side already consumes it and degrades to "awaiting payment" until deployed. Full design + the backend contract + SDK versions + sandbox identities: `docs/decisions/payments.md`. _Tabby (resolved):_ **both** products — "Pay in 4" and "Pay Later" — are carried; each product's enable flag + min/max thresholds are **backend config only** (`tabbyConfig` resolver, one entry per product) — the app hardcodes nothing. The **PDP/cart promo** (`TabbyPromo`) renders one line per enabled, in-range product and the checkout method label reflects the product; all hidden when absent/out of range. The checkout method list still comes only from `available_payment_methods`.
3. **Guest checkout:** Allowed, or must users register before placing an order?
