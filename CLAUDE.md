...
2. **Payment integration (per gateway):** For **N-Genius** and **Tabby** separately — is the redirect/session URL exposed via **GraphQL**, REST only, or a **native mobile SDK**? Is backend resolver work acceptable if GraphQL coverage is missing? For **Tabby** specifically: which products are enabled ("Pay in 4" / "Pay Later"), the min/max order-value thresholds, and is **"Pay in 4" promo messaging on PDP/cart** in scope?
